...
export default function CartRow({ row }) {
  const { byId, incCart, decCart, removeFromCart } = useGame();
  ...
  return (
    <div className="cartRow">
      ...
      <div className="cartRow__mid">
        <button className="btn btn--ghost" onClick={() => decCart(row.id)}>
          –
        </button>
        <span className="cartRow__qty">{row.qty}</span>
        <button className="btn btn--ghost" onClick={() => incCart(row.id)}>
          +
        </button>
      </div>

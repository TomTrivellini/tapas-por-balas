import { useMemo, useState } from "react";
import { useGame } from "../context/GameContext.jsx";
import ItemCount from "./ItemCount.jsx";

function detailLine(entry) {
  if (entry.kind === "recruit") return "Recluta sin stats (por ahora).";
  if (entry.type === "weapon") return `Daño +${entry.effect.value} · Capacidad ${entry.ammoCapacity}`;
  if (entry.type === "gear") return `+${entry.effect.value} ${entry.effect.stat}`;
  if (entry.type === "consumable") return `Recupera +${entry.effect.value} HP`;
  if (entry.type === "ammo") return "Munición para arma";
  return entry.type;
}

export default function ItemDetail({ entry }) {
  const { addToCart, recruitNow } = useGame();
  const [msg, setMsg] = useState("");

  const info = useMemo(() => detailLine(entry), [entry]);

  function onAdd(qty) {
    addToCart(entry.id, qty);
    setMsg(`Agregado al carrito: ${qty} x ${entry.name}`);
  }

  function onRecruit() {
    const r = recruitNow(entry.id);
    if (r.ok) setMsg(`Reclutaste a ${entry.name}`);
    else setMsg("No tenés suficientes tapas para reclutar.");
  }

  return (
    <section className="detail">
      <div className="detail__head">
        <div>
          <h1 className="detail__title">{entry.name}</h1>
          <div className="detail__meta">
            <span className="tag">{entry.category}</span>
            <span className="price">${entry.price}</span>
          </div>
        </div>
      </div>

      <p className="detail__info">{info}</p>

      {entry.kind === "recruit" ? (
        <button className="btn btn--primary" onClick={onRecruit}>
          Reclutar (compra directa)
        </button>
      ) : (
        <div className="detail__buy">
          <ItemCount stock={99} initial={1} onAdd={onAdd} />
        </div>
      )}

      {msg && <p className="notice">{msg}</p>}
    </section>
  );
}
